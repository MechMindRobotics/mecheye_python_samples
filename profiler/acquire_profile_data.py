...
class AcquireProfileData(object):
    ...
    def set_parameters(self):
        ...
        show_error(current_user_set.set_enum_value(ExposureMode.name, ExposureMode.Value_Timed))
        ...
        show_error(current_user_set.set_enum_value(DataAcquisitionTriggerSource.name, DataAcquisitionTriggerSource.Value_Software))
        ...
        show_error(current_user_set.set_enum_value(LineScanTriggerSource.name, LineScanTriggerSource.Value_FixedRate))
        ...
        if self.profiler.get_profiler_info(profiler_info).is_ok():
            ...
            if profiler_info.model == "Mech-Eye LNX 8030":
                show_error(current_user_set.set_enum_value(AnalogGainFor8030.name, AnalogGainFor8030.Value_Gain_1_3))
            else:
                show_error(current_user_set.set_enum_value(AnalogGain.name, AnalogGain.Value_Gain_1_3))
        ...
        show_error(current_user_set.set_enum_value(SpotSelection.name, SpotSelection.Value_Strongest))
        """
        ...
        """
        show_error(current_user_set.set_enum_value(Filter.name, Filter.Value_Mean))
        # Set the window size for mean filtering to WindowSize_2
        show_error(current_user_set.set_enum_value(MeanFilterWindowSize.name, MeanFilterWindowSize.Value_WindowSize_2))

        # Get the line width in the x direction
        error, self.data_width = current_user_set.get_int_value(DataPointsPerProfile.name)
        show_error(error)

        error, self.capture_line_count = current_user_set.get_int_value(ScanLineCount.name)
        show_error(error)
    ...
    def save(self, depth_file_name, intensity_file_name):
        ...
        cv2.imwrite(intensity_file_name, self.total_batch.get_intensity_image().data())
